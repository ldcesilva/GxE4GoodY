# Modified from https://github.com/igorkf/Maize_GxE_Prediction/tree/main/src/evaluate.py
...
def avg_rmse(df_eval, verbose=True):
    rmse_per_group = (
        df_eval
        .groupby('Env')
        .apply(lambda x: root_mean_squared_error(x['ytrue'], x['ypred'])) #squared=False
    )
